@extends('layouts.admin') {{-- 🔥 Berubah menjadi layout admin --}}

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold text-dark"><i class="bi bi-activity text-primary me-2"></i>Monitoring Transaksi SPP</h3>
        <p class="text-muted mb-0">Pantau seluruh aliran dana SPP yang masuk ke sistem secara real-time (Akses Yayasan).</p>
    </div>
    
    {{-- TOMBOL DOWNLOAD EXCEL --}}
    <div>
        {{-- 🔥 Pastikan nama route-nya mengarah ke route admin yayasan --}}
        <a href="{{ route('admin.riwayat.export', request()->all()) }}" class="btn btn-success fw-bold shadow-sm rounded-pill px-4">
            <i class="bi bi-file-earmark-excel-fill me-2"></i>Download Laporan (Excel)
        </a>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        ...
        {{-- FORM FILTER PINTAR --}}
        <form action="{{ route('admin.riwayat.index') }}" method="GET" class="row g-2 mb-4 bg-light p-3 rounded-4 border">
            <div class="col-md-4">
            ...
            <div class="col-md-3 d-flex align-items-end gap-2">
                ...
                <a href="{{ route('admin.riwayat.index') }}" class="btn btn-light w-100 fw-bold shadow-sm rounded-3 text-muted border">Reset</a>
            </div>
        </form>
