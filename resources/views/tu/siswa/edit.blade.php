...
<div class="d-flex align-items-center mb-4 gap-3">
    ...
    <div>
        <h3 class="fw-bolder text-dark mb-0" style="letter-spacing: -0.5px;">Edit Data Siswa</h3>
        <p class="text-secondary mb-0" style="font-size: 0.9rem;">Perbarui biodata atau status akademik siswa.</p>
    </div>
</div>
...
<div class="card border-0 rounded-4 bg-white mb-5" style="box-shadow: 0 4px 24px rgba(0,0,0,0.03); border: 1px solid #e2e8f0 !important;">
    <div class="card-body p-4 p-md-5">
        <form action="{{ route('tu.siswa.update', $siswa->id) }}" method="POST">
            ...
            {{-- 🔥 AREA BERBAHAYA: SAKLAR STATUS SISWA 🔥 --}}
            <div class="alert bg-danger bg-opacity-10 border-0 border-start border-danger border-4 rounded-3 p-4 mb-5 shadow-sm">
                <div class="d-flex align-items-center mb-3">
                    <i class="bi bi-exclamation-octagon-fill text-danger fs-3 me-3"></i>
                    <div>
                        <h6 class="fw-bold text-danger mb-0">Status Akademik Siswa</h6>
                        <small class="text-danger opacity-75">Perhatian: Mengubah status akan mempengaruhi akses login siswa.</small>
                    </div>
                </div>
                <div class="bg-white p-2 rounded-3 border border-danger border-opacity-25">
                    <select name="status_siswa" class="form-select form-select-lg border-0 shadow-none fw-bolder" required style="color: #0f172a; background-color: transparent;">
                        <option value="Aktif" {{ $siswa->status_siswa == 'Aktif' ? 'selected' : '' }}>🟢 AKTIF (Masih Bersekolah)</option>
                        <option value="Pindah" {{ $siswa->status_siswa == 'Pindah' ? 'selected' : '' }}>🔴 PINDAH SEKOLAH / KELUAR</option>
                    </select>
                </div>
            </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="number" name="nisn" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary fw-bold text-dark" value="{{ $siswa->nisn }}" required>
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="nama_lengkap" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary fw-bold text-dark" value="{{ $siswa->nama_lengkap }}" required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="tempat_lahir" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" value="{{ $siswa->tempat_lahir }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold text-secondary small text-uppercase" style="letter-spacing: 0.5px;">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" value="{{ $siswa->tanggal_lahir }}">
                </div>
                
                {{-- Data Email dan WA Siswa --}}
                <div class="col-md-6">
                    <label class="form-label fw-semibold text-secondary small text-uppercase" style="letter-spacing: 0.5px;">Email <span class="text-muted fw-normal text-capitalize">(Opsional)</span></label>
                    <input type="email" name="email" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" value="{{ $siswa->user->email ?? '' }}">
                    <small class="text-muted mt-1 d-block" style="font-size: 0.75rem;">Akan memperbarui email .</small>
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="no_hp_siswa" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-success" value="{{ $siswa->no_hp_siswa }}">
                </div>
            
                <div class="col-md-12">
                    <label class="form-label fw-semibold text-secondary small text-uppercase" style="letter-spacing: 0.5px;">Alamat Lengkap</label>
                    <textarea name="alamat" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" rows="2">{{ $siswa->alamat }}</textarea>
                </div>
            </div>
            ...
            <div class="row g-4 mb-5">
                <div class="col-md-6">
                    ...
                    <input type="text" name="nama_orang_tua" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" value="{{ $siswa->nama_orang_tua }}">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="no_hp_ortu" class="form-control form-control-lg bg-light fs-6 border-light shadow-none focus-ring-primary" value="{{ $siswa->no_hp_ortu }}">
                </div>
            </div>
