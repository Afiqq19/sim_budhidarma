...
<div class="mb-4">
    ...
    <h3 class="fw-bold text-dark">Edit Mata Pelajaran</h3>
</div>
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        <form action="{{ route('tu.mapel.update', $mapel->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="kelompok" class="form-select" required>
                        <option value="A" {{ $mapel->kelompok == 'A' ? 'selected' : '' }}>Kelompok A (Muatan Nasional)</option>
                        <option value="B" {{ $mapel->kelompok == 'B' ? 'selected' : '' }}>Kelompok B (Muatan Kewilayahan)</option>
                        <option value="C" {{ $mapel->kelompok == 'C' ? 'selected' : '' }}>Kelompok C (Muatan Peminatan Kejuruan)</option>
                        <option value="C2" {{ $mapel->kelompok == 'C2' ? 'selected' : '' }}>Kelompok C2 (Dasar Program Keahlian)</option>
                        <option value="C3" {{ $mapel->kelompok == 'C3' ? 'selected' : '' }}>Kelompok C3 (Kompetensi Keahlian)</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label fw-bold">Nilai KKM Minimal</label>
                    <input type="number" name="kkm" class="form-control" value="{{ $mapel->kkm }}" min="0" max="100" required>
                </div>
            </div>
